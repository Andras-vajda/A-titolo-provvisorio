 ...
 * test_known_cases.....................: 6
 * main.................................: 10
 * Complessita ciclomatica media.......: 7.55
 * Complessita ciclomatica massima.....: 18 (round_robin)
 * 
 * ### Metriche di Halstead:
 * Operatori unici (n1)................: 39
 * Operandi unici (n2).................: 135
 * Occorrenze totali operatori (N1)....: 968
 * Occorrenze totali operandi (N2).....: 1186
 * Vocabolario (n).....................: 174
 * Lunghezza (N).......................: 2154
 * Volume (V)..........................: 16,182
 * Difficolta (D)......................: 171.21
 * Sforzo (E)..........................: 2,770,520
 * Tempo richiesto (T).................: 153,918 secondi (~42.8 ore)
 * Bug stimati.........................: 5.39
 * 
 * ### Metriche di Manutenibilita:
 * Linee di codice (LOC)...............: 629
 * Linee di commento...................: 18
 * Rapporto commento/codice............: 0.029
 * Indice di manutenibilita............: 68.74 (scala 0-100)
 * 
 ...
 * Complessita cognitiva totale........: 142
 * Media per funzione..................: 12.91
 * Funzione piu complessa..............: `solve` (36)
 * 
...
impl FrobeniusSolver {
    ...
    fn gcd_of_vec(&self, coins: &[BigInt]) -> BigInt {
        ...
        for i in 1..coins.len() {
            ...
            // Ottimizzazione: termina prima se il MCD e' 1
    ...
    fn round_robin(&self, coins: &mut Vec<BigInt>) -> Result<BigInt, String> {
        ...
        // Estrai la moneta piu piccola
        let a: u64 = match coins[0].to_u64() {
            Some(val) => val,
            None => return Err("La moneta piu piccola e' troppo grande per u64".to_string()),
        };
        
        if a > 10_000_000 {
            return Err("La moneta piu piccola e' troppo grande per l'algoritmo Round-Robin".to_string());
        }
        ...
        if let Some(max) = max_value {
            let result = max - BigInt::from_u64(a).unwrap();
            Ok(result)
        } else {
    ...
    fn dp_algorithm(&self, coins: &[BigInt]) -> Result<BigInt, String> {
        ...
        reachable.set(0, true);  // 0 e' sempre raggiungibile
        ...
        if bound_val > 1_000_000 && self.thread_count > 1 && coins.len() >= 3 {
            ...
            for &coin_val in &coin_vals {
                ...
                let thread_bound_val = bound_val;
                
                let handle = thread::spawn(move || {
                    let mut local_reachable = bitvec![u64, Lsb0; 0; thread_bound_val as usize + 1];
                    local_reachable.set(0, true);
                    
                    // Calcola i valori raggiungibili localmente per questa moneta
                    for i in coin_val..=thread_bound_val {
                        if local_reachable[i as usize - coin_val as usize] {
                            local_reachable.set(i as usize, true);
                        }
                    }
                    
                    // Combina il risultato locale con quello globale
            ...
            // Approccio sequenziale per bound piu piccoli
            for &coin_val in &coin_vals {
                for i in coin_val..=bound_val {
                    if reachable[i as usize - coin_val as usize] {
        ...
        for i in (0..=bound_val).rev() {
            if !reachable[i as usize] {
                return Ok(BigInt::from_u64(i).unwrap());
            }
        }
        
        // Se tutti i valori sono raggiungibili, allora il numero di Frobenius e' -1
        Ok(BigInt::from_i64(-1).unwrap())
    }
    ...
    pub fn solve(&mut self, mut coins: Vec<BigInt>) -> Result<BigInt, String> {
        ...
        if coins.is_empty() {
            return Err("L'insieme delle monete non puo essere vuoto".to_string());
        }
    ...
    pub fn test_known_cases(&mut self) -> bool {
        ...
        for (coins_vec, expected) in test_cases {
            ...
            let result = match self.solve(coins) {
                Ok(val) => {
...
fn main() {
    ...
    if verbose {
        println!("Modalita verbose attiva");
    }
    ...
    // Performance test con numeri piu grandi
    let challenge = vec![
        BigInt::from(31u8),
        BigInt::from(41u8),
        BigInt::from(59u8),
    ...
    // Test con numeri ancora piu grandi
